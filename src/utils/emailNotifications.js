...
export const sendGracePeriodEmail = async ({
  ...
  try {
    const formattedEndDate = new Date(gracePeriodEndDate).toLocaleDateString(
      "en-US",
      {
        year: "numeric",
        month: "long",
    ...
    const emailData = {
      ...
      data: {
        ...
        daysRemaining: Math.ceil(
          (new Date(gracePeriodEndDate) - new Date()) / (1000 * 60 * 60 * 24),
        ),
      },
    };
...
export const createReceipt = async (receiptData) => {
  try {
    ...
    // Support both userId (new) and teacherId (legacy) for backwards compatibility
    const ownerId = userId || teacherId;
