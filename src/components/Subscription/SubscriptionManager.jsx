...
const SubscriptionManager = () => {
  ...
  // Handle successful payment from Monnify
  const handlePaymentComplete = async (paymentData) => {
    try {
      ...
      // Note: verifyWithMonnify is false because the Monnify SDK already verified the payment
      const result = await handlePaymentSuccess({
        ...
        monnifyResponse: paymentData.monnifyResponse,
        verifyWithMonnify: false,
      });
            ...
            {subscription.planTier !== "free" && subscription.expiryDate && (
              <div className="text-right">
                ...
                <p className="text-sm font-semibold text-gray-900">
                  {new Date(
                    subscription.expiryDate.seconds * 1000,
                  ).toLocaleDateString()}
                </p>
              </div>
            )}
